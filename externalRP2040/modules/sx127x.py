...
class RADIO:
    def __init__(self,
                 ...
                 gpio = {'led':    17,    # blue LED GPIO number on board
                         'reset':  1,    # reset pin from GPIO5 (or may be None)
                         'dio0':   0,    # DIO0 line to GPIO4
                         'cs':     10,   # SPI CS
                         'sck':    2,   # SPI SCK
                         'mosi':   3,   # SPI MOSI
                         'miso':   4},  # SPI MISO
        ...
        self.spi = SPI(0, baudrate=spi_baudrate, polarity=0, phase=0,
                        sck=Pin(gpio['sck']),
                        mosi=Pin(gpio['mosi']),
                        miso=Pin(gpio['miso']))
