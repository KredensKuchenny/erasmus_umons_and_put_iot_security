...
from modules.bmp280 import BMP280I2C

SERIAL_NUMBER = "ext001b"
...
def get_measurment(bmp280, adc, deep_sleep, number_of_measurements=1):
    ...
    for i in range(number_of_measurements):
        bmp280_readout = bmp280.measurements
        battery_voltage = adc.read_u16() * 5 / 65535

        sleep(0.2)

        data_array[0] += float(bmp280_readout["t"])
        data_array[1] += float(bmp280_readout["p"])
        data_array[2] += float(battery_voltage)

        print("i number:", i, ", measurments:", data_array)

        if i + 1 == number_of_measurements:
            data["bmp280"]["temp"] = round((data_array[0] / number_of_measurements), 2)
            data["bmp280"]["press"] = round((data_array[1] / number_of_measurements), 2)
...
i2c = I2C(1, scl=Pin(27), sda=Pin(26), freq=10000)  # I2C declaration
bmp280 = BMP280I2C(0x76, i2c)  # setting BMP280 sensor
...
if MODE == 1:
    ...
    while True:
        ...
        get_measurment(bmp280, adc, current_sleep_time, 3)
        sleep(5)
